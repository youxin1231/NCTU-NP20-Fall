...
def read_tcp(conn):
    while True:
        ...
        if data[1] == 'login':
            ...
            Comment(data, conn)
        elif data[1] == 'create-chatroom':
            Create_chatroom(data, conn)
        elif data[1] == 'join-chatroom':
            Join_chatroom(data, conn)
        elif data[1] == 'attach':
            Attach(data, conn)
        elif data[1] == 'restart-chatroom':
            Restart_chatroom(data, conn)
        elif data[0] == 'close-chatroom':
            Close_chatroom(data, conn)
        elif data[1] == 'exit':
            if len(data) == 2:
...
def read_udp():
    while True:
        ...
        elif data[1] == 'list-chatroom':
            List_chatroom(data, udp_addr)
...
def Register(data, udp_addr):
    if len(data) == 5:
        ...
        db = datadb.cursor()
        db.execute(
            '''SELECT COUNT(Username) FROM USERS WHERE Username = ?''', (data[2],))
        count = db.fetchone()
        if count[0] == 0:
...
def Login(data, conn):
    if len(data) == 4:
        ...
        db = datadb.cursor()
        db.execute(
            '''SELECT Client_name FROM CLIENT WHERE Random_id = ?''', (data[0],))
        ...
        if client_name is not None:
            ...
            db.execute(
                '''SELECT Username FROM USERS WHERE Username = ? AND Password = ?''', (data[2], data[3],))
            fetch = db.fetchone()
            if fetch is not None:
                client_name = fetch[0]
                random_id = random.randint(1, 2147483647)
                db.execute(
                    '''REPLACE INTO CLIENT (Random_id , Client_name) VALUES (? , ?)''', (random_id, client_name))
                datadb.commit()
...
def Logout(data, conn):
    if len(data) == 2:
        ...
        client_name = db.fetchone()
        if client_name is not None:
            if [client_name[0], 'open'] in chatroom:
                conn.sendall(
                    "Please do “attach” and “leave-chatroom” first.".encode())
            else:
                db.execute(
                    '''DELETE FROM CLIENT WHERE Random_id = ?''', (data[0], ))
                datadb.commit()
...
def Create_board(data, conn):
    if len(data) == 3:
        ...
        if client_name is not None:
            db.execute(
                '''SELECT COUNT(Board_name) FROM BOARD WHERE Board_name = ?''', (data[2],))
            count = db.fetchone()
            if count[0] == 0:
                db.execute(
                    '''INSERT INTO BOARD (Board_name , Moderator) VALUES (? , ?)''', (data[2], client_name[0]))
    ...
    if len(data) >= 7 and data[3] == '--title' and data.index('--content') >= 5:
        ...
        if client_name is not None:
            ...
            if count[0] != 0:
                ...
                cur_post = [board_name, str(
                    post_id), title, client_name[0], time, content, []]
        ...
        conn.sendall(
            "Usage: create-post <board-name> --title <title> --content <content>".encode())
    datadb.close()
    lock.release()
...
def Update_post(data, conn):
    ...
    if len(data) >= 5 and (data[3] == '--title' or data[3] == '--content'):
        ...
        conn.sendall(
            "Usage: update-post <post-S/N> --title/content <new>".encode())
    lock.release()
...
def Create_chatroom(data, conn):
    if len(data) == 3:
        ...
        if client_name is not None:
            if [client_name[0], 'open'] in chatroom:
                conn.sendall("User has already created the chatroom.".encode())
            else:
                chatroom.append([client_name[0], 'open'])
                port_list.append([client_name[0], data[2]])
                string = "start to create chatroom… " + client_name[0]
                conn.sendall(string.encode())
        else:
            conn.sendall("Please login first.".encode())
        datadb.close()
    else:
        conn.sendall("Usage: create-chatroom <port>".encode())


def List_chatroom(data, udp_addr):
    if len(data) == 2:
        datadb = sqlite3.connect("data.db")
        db = datadb.cursor()
        db.execute(
        ...
        client_name = db.fetchone()
        if client_name is not None:
            string = '{:<20} {:<10}'.format('Chatroom_name', 'Status')
            for r in chatroom:
                string += '\n' + '{:<20} {:<10}'.format(r[0], r[1])
            udp.sendto(string.encode(), udp_addr)
        else:
            udp.sendto("Please login first.".encode(), udp_addr)
        datadb.close()
    else:
        udp.sendto("Usage: list-chatroom".encode(), udp_addr)


def Join_chatroom(data, conn):
    if len(data) == 3:
        datadb = sqlite3.connect("data.db")
        db = datadb.cursor()
        db.execute(
        ...
        client_name = db.fetchone()
        if client_name is not None:
            if [data[2], 'open'] in chatroom:
                for sublist in port_list:
                    if sublist[0] == data[2]:
                        port_num = sublist[1]
                        break
                string = "Join success. " + port_num + " " + client_name[0]
                conn.sendall(string.encode())
            else:
                conn.sendall(
                    "The chatroom does not exist or the chatroom is close.".encode())
        else:
            conn.sendall("Please login first.".encode())
        datadb.close()
    else:
        conn.sendall("Usage: join-chatroom <chatroom_name>".encode())


def Attach(data, conn):
    if len(data) == 2:
        datadb = sqlite3.connect("data.db")
        db = datadb.cursor()
        db.execute(
        ...
        client_name = db.fetchone()
        if client_name is not None:
            if any(client_name[0] in sublist for sublist in chatroom):
                if [client_name[0], 'open'] in chatroom:
                    for sublist in port_list:
                        if sublist[0] == client_name[0]:
                            port_num = sublist[1]
                            break
                    string = "Attach success. " + \
                        port_num + " " + client_name[0]
                    conn.sendall(string.encode())
                else:
                    conn.sendall("Please restart-chatroom first.".encode())
            else:
                conn.sendall("Please create-chatroom first.".encode())
        else:
            conn.sendall("Please login first.".encode())
        datadb.close()
    else:
        conn.sendall("Usage: attach".encode())


def Restart_chatroom(data, conn):
    if len(data) == 2:
        datadb = sqlite3.connect("data.db")
        db = datadb.cursor()
        db.execute(
        ...
        if client_name is not None:
            if any(client_name[0] in sublist for sublist in chatroom):
                if [client_name[0], 'open'] in chatroom:
                    conn.sendall("Your chatroom is still running.".encode())
                else:
                    for sublist in chatroom:
                        if sublist[0] == client_name[0]:
                            sublist[1] = 'open'
                        for sublist in port_list:
                            if sublist[0] == client_name[0]:
        ...
        conn.sendall("Usage: restart-chatroom".encode())


def Close_chatroom(data, conn):
    for sublist in chatroom:
        if sublist[0] == data[1]:
            sublist[1] = 'close'
            break
